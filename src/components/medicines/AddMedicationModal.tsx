...
import { addFamilyMedicineAndSchedules } from "@/features/family/actions";
...
interface AddMedicationModalProps {
  ...
  /** When provided, medicine will be added to this family for the target member */
  familyContext?: {
    familyId: string;
    targetUserId: string;
  };
...
/**
 * AddMedicationModal — wraps the Add Medicine form content in a shadcn Dialog.
 * Supports both personal medicine (no familyContext) and family medicine.
 * UI is identical to the original AddMedicineForm.
 */
export function AddMedicationModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    let result;
    if (familyContext) {
      result = await addFamilyMedicineAndSchedules(
        familyContext.familyId,
        familyContext.targetUserId,
        {
          ...form,
      ...
      result = await addMedicineAndSchedules(
        {
          ...form,
          category_id: categoryId || undefined,
          stock_qty: stockQty,
      ...
      <DialogContent className="sm:max-w-[480px] p-0 gap-0 overflow-hidden">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-border/40">
          ...
          <DialogDescription className="text-sm text-muted-foreground">
            {familyContext
              ? "Add a medication for this family member."
              : "Add a new medication to your personal list."}
          </DialogDescription>
        </DialogHeader>

        <ScrollArea className="max-h-[70vh]">
          <form
            id="add-medication-modal-form"
          ...
          >
            ...
            <div className="flex flex-col gap-1 pt-2 border-t border-border/30">
              ...
              <div className="flex flex-col gap-2">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger className="inline-flex h-11 w-full items-center justify-between rounded-2xl border border-input bg-secondary/30 px-4 text-sm font-medium outline-none transition-all hover:bg-secondary/40 hover:text-accent-foreground cursor-pointer focus:ring-4 focus:ring-primary/10">
                    <span>
                      {FREQUENCY_LABELS[freqType] || "Select frequency"}
                    </span>
                    <svg
                      viewBox="0 0 24 24"
                      fill="none"
            ...
            <div className="grid grid-cols-2 gap-3 mt-3">
              ...
              <div className="flex flex-col gap-2">
                ...
                <Popover>
                  <PopoverTrigger
                    className="flex h-11 w-full rounded-2xl border border-input bg-secondary/30 px-3 py-2 text-sm font-medium transition-all outline-none focus-visible:border-primary focus-visible:ring-4 focus-visible:ring-primary/10 hover:bg-secondary/50 justify-start text-left items-center gap-2 cursor-pointer"
                  >
              ...
              <div className="flex flex-col gap-2">
                ...
                <Popover>
                  <PopoverTrigger
                    className={cn(
                      "flex h-11 w-full rounded-2xl border border-input bg-secondary/30 px-3 py-2 text-sm font-medium transition-all outline-none focus-visible:border-primary focus-visible:ring-4 focus-visible:ring-primary/10 hover:bg-secondary/50 justify-start text-left items-center gap-2 cursor-pointer",
            ...
            <div className="flex flex-col gap-1 pt-2 border-t border-border/30">
              ...
              {/* Stock Qty + Expiration Date */}
              <div className="grid grid-cols-2 gap-3">
                {/* Stock Quantity */}
                <div className="flex flex-col gap-2">
                  <label
                    htmlFor="modal-med-stock"
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Popover>
                    ...
                    <PopoverContent
                      className="w-auto p-0 rounded-2xl border-border/50 shadow-xl"
                      align="start"
                    >
                      <Calendar
                        mode="single"
            ...
            {successName && (
              ...
              >
                ✓ <span className="font-medium">{successName}</span> added
                successfully!
              </div>
            )}
          </form>
        ...
        <div className="px-6 py-4 border-t border-border/40 bg-background/80 backdrop-blur-sm">
          <Button
            id="add-medication-modal-submit"
            type="submit"
            form="add-medication-modal-form"
            disabled={loading}
            className="w-full h-12 rounded-2xl text-base font-bold shadow-lg shadow-primary/20 transition-all cursor-pointer bg-primary"
          >
